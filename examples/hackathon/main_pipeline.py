...
from xml_parser import convert_dates_in_schedule, process_schedule, schedule_csv_to_xml, \
    get_works_info
...
def single_scheduling(data_path: str, output_path: Optional[str] = None,
    ...
    schedule_df = convert_dates_in_schedule(schedule.full_schedule_df, start_date)
    schedule_df.to_csv('all_tasks_df.csv')

    pure_df = convert_dates_in_schedule(schedule.pure_schedule_df, start_date)
    pure_df.to_csv('pure_tasks_df.csv')


    if output_path is not None:
        schedule_df.to_csv(os.path.join(output_path, 'schedule_time.csv'), index=0)
        _, *structure_info = get_works_info(data_path)
...
if __name__ == "__main__":
    ...
    _, *structure_info = get_works_info(filepath)

    # schedule_df = pd.read_csv('output/schedule_time.csv')
    schedule_df = raw_project_schedule.full_schedule_df
    # pure_schedule_df = raw_project_schedule.pure_schedule_df

    # schedule_df.to_csv('all_tasks_df.csv', index=False)
    # pure_schedule_df.to_csv('pure_tasks_df.csv', index=False)

    # _, *structure_info = get_project_works_structure(filepath)
    schedule_csv_to_xml(*process_schedule(schedule_df, structure_info),
                        filepath, os.path.join('for_final', 'schedule_time_optimization_no_lags.xml'))
