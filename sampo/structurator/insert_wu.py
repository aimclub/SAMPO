...
def insert_work_unit(original_wg: WorkGraph, inserted_wu: WorkUnit,
    ...
    copied_nodes, original_old_to_new_ids = prepare_work_graph_copy(original_wg)
...
#                      parents_edges: list[tuple[GraphNode, float, EdgeType]],
#                      children_edges: list[tuple[GraphNode, float, EdgeType]]) -> WorkGraph:
#     copied_wg, old_to_new_ids = prepare_work_graph_copy(original_wg)
#     new_nodes = {node.id: node for node in copied_wg.nodes}
#
#     new_parents_edges = [(new_nodes[old_to_new_ids[parent.id]], lag, edge_type)
#                          for parent, lag, edge_type in parents_edges]
#     new_children_edges = [(new_nodes[old_to_new_ids[child.id]], lag, edge_type)
#                           for child, lag, edge_type in children_edges]
#
#     new_node = GraphNode(inserted_wu, [])
#     new_node.add_parents(new_parents_edges)
...
#         child.add_parents([(new_node, lag, edge)])
#
#     new_start = new_nodes[old_to_new_ids[original_wg.start.id]]
#     new_finish = new_nodes[old_to_new_ids[original_wg.finish.id]]
#
#     return WorkGraph(new_start, new_finish)
