...
from schemas.schedule import ResourceSchedule, ScheduleWorkDict, Schedule
...
from utils.visualization.work_graph import SIZE_LIMIT, DPI_LIMIT, DEFAULT_DPI
...
def get_schedule_df(schedule: ScheduleWorkDict, fig_type: EmploymentFigType, project_start: datetime) -> DataFrame:
    if fig_type == EmploymentFigType.DateLabeled:
        max_time = max(schedule.values(), key=lambda x: x.finish_time).finish_time + 1
