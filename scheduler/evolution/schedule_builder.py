...
from schemas.schedule import ScheduleWorkDict, Schedule
from scheduler.evolution.converter import convert_schedule_to_chromosome, convert_chromosome_to_schedule
from scheduler.evolution.operators import init_toolbox, ChromosomeType
...
def build_schedule(wg: WorkGraph,
    ...
    for work_index, node in index2node.items():
        for req in node.work_unit.worker_reqs:
            worker_index = worker_name2index[req.type]
    ...
    toolbox = init_toolbox(wg, contractors, agents, index2node,
                           work_id2index, worker_name2index, resources_border,
                           init_chromosomes,
    ...
    while g < generation_number:
        ...
        # workers type for changing
        workers = rand.sample(range(len(worker_name2index)), number_of_type_for_changing)

        # resources mutation
        for worker in workers:
            low = resources_border[0, worker]
            up = resources_border[1, worker]
            for mutant in offspring:
                if rand.random() < mutpb_res:
                    toolbox.mutate_resources(mutant[0], low=low, up=up, type_of_worker=worker)
                    del mutant.fitness.values

        # for the crossover, we use those types that did not participate in the mutation
        workers_for_mate = list(set(list(range(len(worker_name2index)))) - set(workers))
        # crossover
