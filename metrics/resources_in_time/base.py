...
from utils.visualization.resources import get_workers_intervals, SPLITTER
...
def get_schedule_with_time(scheduler: Scheduler, wg: WorkGraph,
    ...
    scheduled_works = scheduler.schedule(wg, agents_to_contractors(agent_counts, agent_names))[0]
...
def find_min_agents(wg: WorkGraph, max_workers: int) -> Tuple[np.array, List[str]]:
    ...
    for node in wg.nodes:
        for req in node.work_unit.worker_reqs:
            min_agents[req.type] = min(min_agents[req.type], req.min_count)
...
def init_borders(wg: WorkGraph, scheduler: Scheduler, deadline: Time,
                 worker_factor: int,
                 max_workers: int,
                 right_agents: AgentsDict or None) -> Tuple[Optional[np.ndarray], Optional[np.ndarray], List[str]]:
...
def prepare_answer(counts: np.array, agent_names: List[str], wg: WorkGraph, scheduler: Scheduler, dry_resources: bool):
