...
import generator.config.gen_counts as gen_c
import generator.config.worker_req as wr
from generator.config.worker_req import scale_reqs
from schemas.interval import Interval
from schemas.utils import uuid_str
from schemas.works import WorkUnit
from schemas.graph import GraphNode, EdgeType
...
def add_addition_work(probability: float, rand: Optional[Random] = None) -> bool:
    return Interval(0, 1).float(rand) <= probability
...
def get_boreholes_equipment_general(parents: List[GraphNode], cluster_name: str, pipes_count: int, masts_count: int,
    ...
    for i in range(pipes_count):
        dist = gen_c.DIST_BETWEEN_BOREHOLES.float(rand)
        dists_sum += dist
...
def get_cluster_works(root_node: GraphNode, cluster_name: str, pipe_nodes_count: int,
    ...
    dist_to_parent = None
    if not is_slave:
        dist_to_parent = gen_c.DIST_TO_PARENT.float(rand)
    ...
    dist_line = gen_c.ONE_SECTION_PIPE.float(rand)
    power_lines = get_power_lines(preparation_stage, cluster_name, dist_line, dist_high_line=dist_to_parent, rand=rand)

    pipe_dists = [gen_c.ONE_SECTION_PIPE.float(rand) for _ in range(pipe_nodes_count + 1)]
