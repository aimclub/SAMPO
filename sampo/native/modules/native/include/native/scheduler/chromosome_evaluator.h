...
private:
    ...
    static void updateTimeline(
        ...
        for (int worker = 0; worker < teamSize; worker++) {
            ...
            while (need_count > 0) {
                int next_count =
                    worker_timeline[worker_timeline.size() - 1].second;
                ...
                if (worker_timeline.size() == 1 && need_count > 0) {
                    cerr << "---- Empty worker_timeline for worker " << worker
                         << " and contractor " << contractor << endl
                         << flush;
                    return;
                }
            ...
            size_t ind = worker_timeline.size() - 1;
            while (ind > 0
                   && worker_timeline[ind].first
                          > worker_timeline[ind - 1].first) {
    ...
    int schedule(
        ...
        for (int dep_node : inseparables[nodeIndex]) {
            ...
            int workingTime = calculate_working_time(
                chromosome_ind, dep_node, nodeIndex, resources, teamSize
            );
    ...
    inline Timeline createTimeline() {
        ...
        for (int contractor = 0; contractor < workers.size(); contractor++) {
            ...
            for (int worker = 0; worker < workers[0].size(); worker++) {
                timeline[contractor][worker].emplace_back(
                    0, workers[contractor][worker]
                );
            }
        }
...
public:
    ...
    explicit ChromosomeEvaluator(EvaluateInfo *info)
        ...
        this->numThreads =
            this->usePythonWorkEstimator ? 1 : omp_get_num_procs();
        ...
        if (info->useExternalWorkEstimator) {
            ...
            this->timeEstimator =
                new DefaultWorkTimeEstimator(minReqNames, maxReqNames);
        }
    }
...
#pragma omp parallel for shared(chromosomes) default(none)                     \
    ...
    int evaluate(int chromosome_ind, Chromosome *chromosome) {
        ...
        for (int i = 0; i < chromosome->numWorks(); i++) {
            ...
            int st = findMinStartTime(
                workIndex,
                contractor,
                team,
                chromosome->numResources(),
                completed,
                timeline
            );
            if (st == TIME_INF) {
