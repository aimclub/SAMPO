...
def test_work_graph_csv_parser():
    try:
        print(sys.path)
        works_info = CSVParser.read_graph_info(project_info=os.path.join(sys.path[0], 'tests/parser/test_work_info.csv'),
                                               history_data=os.path.join(sys.path[0], 'tests/parser/test_history.csv'),
                                               change_base_on_history=True)
        works_info.to_csv(os.path.join(sys.path[0], 'tests/parser/repaired_connections.csv'), sep=';')
