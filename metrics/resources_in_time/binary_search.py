...
from schemas.contractor import Contractor, AgentsDict, get_agents_from_contractors
...
class BinarySearchOptimizer(ResourceOptimizer):
    ...
    def optimize(self, wg: WorkGraph, deadline: Time, agents: Optional[AgentsDict] = None,
        ...
        return optimize_func(wg, self.scheduler, deadline, self.worker_factor, self.max_workers, agents,
...
def union_search(wg: WorkGraph, scheduler: Scheduler, deadline: Time,
                 worker_factor: Optional[int] = 1000,
                 max_workers: Optional[int] = 1000,
                 right_agents: Optional[AgentsDict] = None,
...
def item_by_item_fast_init_search(wg: WorkGraph, scheduler: Scheduler, deadline: Time,
    ...
    contractor, max_time = union_search(wg, scheduler, deadline, worker_factor, max_workers, right_agents,
                                        dry_resources=False)
    if contractor is None:
        return None, None
    agents = get_agents_from_contractors([contractor])
