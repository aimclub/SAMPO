...
from generator.config.gen_counts import MIN_GRAPH_COUNTS, ADDITION_CLUSTER_PROBABILITY, GRAPH_COUNTS, \
    MAX_BOREHOLES_PER_BLOCK, BRANCHING_PROBABILITY
from generator.pipeline.cluster import get_start_stage, get_cluster_works, get_finish_stage, add_addition_work
from schemas.graph import GraphNode, WorkGraph
from schemas.utils import count_node_ancestors
...
def get_small_graph(cluster_name: Optional[str] = 'C1', rand: Optional[Random] = None) -> WorkGraph:
    ...
    s = get_start_stage()
    pipe_nodes_count = MIN_GRAPH_COUNTS['pipe_nodes'].int(rand)
    pipe_net_count = MIN_GRAPH_COUNTS['pipe_net'].int(rand)
    light_masts_count = MIN_GRAPH_COUNTS['light_masts'].int(rand)
    borehole_count = MIN_GRAPH_COUNTS['borehole'].int(rand)
...
def get_cluster_graph(root_node: GraphNode, cluster_name: str, pipe_nodes_count: Optional[int] = None,
    ...
    pipe_nodes_count = pipe_nodes_count or GRAPH_COUNTS['pipe_nodes'].int(rand)
    pipe_net_count = pipe_net_count or GRAPH_COUNTS['pipe_net'].int(rand)
    light_masts_count = light_masts_count or GRAPH_COUNTS['light_masts'].int(rand)
    if borehole_counts is None:
        counts = GRAPH_COUNTS['borehole'].int(rand)
