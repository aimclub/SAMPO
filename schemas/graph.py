...
from schemas.serializable import JSONSerializable, T, JS
...
class GraphNode(JSONSerializable['GraphNode']):
    ...
    def update_work_unit(self, work_unit: WorkUnit):
        self._work_unit = work_unit

    def add_parents(self, parent_works: List['GraphNode'] or List[Tuple['GraphNode', float, EdgeType]]):
        """
    ...
    def traverse_children(self, topologically: bool = False):
        ...
        while len(vertexes_to_visit) > 0:
            ...
            if topologically and any(p not in visited_vertexes for p in v.parent_nodes):
    ...
    def inseparable_parent(self) -> Optional['GraphNode']:
        inseparable_parents = list([x.finish for x in self._parent_edges if x.type == EdgeType.InseparableFinishStart])
        return inseparable_parents[0] if inseparable_parents else None

    # TODO Maybe should be set
    @property
    def parent_nodes(self) -> List['GraphNode']:
        return [edge.start for edge in self._parent_edges]

    # TODO Maybe should be set
    @property
    def children(self) -> List['GraphNode']:
        return [edge.finish for edge in self._children_edges]
